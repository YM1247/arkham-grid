...
個別咒文 tooltip 只顯示 MP 成本；「MP 不足時由 Sanity 等量代付」屬於共通戰鬥規則，只在教學、圖例與實際消除預覽呈現，避免每張卡重複。消除預覽會列出所有即將觸發的咒文、實際效果與總支付，致死風險以紅字警告。瘋狂「儀式執著」會讓咒文成本增加 1，代付時也包含增加後的完整成本。戰鬥 HUD 顯示目前階段、效果與戰鬥回合時限，不再佔用版面顯示「最近理智變化」；RunState 仍保存完整來源歷史。正式 Sanity 來源包含咒文代付、戰鬥逾時、死盤、敵人意圖、事件與休息。

每份戰鬥報告會依來源前綴彙總實際 Sanity 損失；若 Sanity 歸零，結算會顯示最後致死來源，近期 Run 紀錄與無 UI 模擬也會保存同一分類。首次進入限時戰鬥時另顯示一次非阻斷提示，之後由 Meta 教學進度避免重複出現。
