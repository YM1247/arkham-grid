...
- `ProfileSaveService` 將 `SettingsState` 與共享 `MetaState` 分成兩份獨立版本檔案，沿用原子寫入與 last-known-good 備份；設定保存語言、視窗、音量與匿名回報偏好，Meta 保存共享貨幣、Run 統計及職業／方塊／咒文解鎖。
...
RunState -> BattleStartInput -> BattleManager -> BattleResult -> RunManager
   ...
   +-> BoardModel / effect cells / hand / pools / progress
```
