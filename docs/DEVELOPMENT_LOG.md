# 開發紀錄

## 2026-09-19：石板尺寸、消除時序與場景轉場修正

- 側邊三張手牌改以 46px 棋盤原尺寸繪製；形狀區加寬，卡面文字右移，垂直槽距保留四格長石板的溢出空間，拖曳預覽仍與盤面一比一。
- 消除順序統一為「同步閃爍 → 清除棋格 → 逐一顯示咒文名稱並結算效果」；效果格在清除前先保存，因此盤面消失後仍維持固定觸發順序。
- 死盤檢查新增排程鎖；補牌所建立的延遲檢查若遇到消除中會取消，僅在整段清除與咒文結算完成後重新檢查及播放警示。
- 修正盤面咒文徽章同時設定 46px 最小尺寸與 40px 實際尺寸造成的右下偏移，現在四邊固定保留 3px 並精確置中。
- 敵人名稱置中；鎖定四角向立繪內縮並縮短角線，和名稱／HP 列保留清楚間距。
- 地圖節點移除 hover tooltip；主選單、地圖、事件、獎勵與結算改以共用淡入淡出銜接。

## 2026-09-19：地圖路線疏密調整

- 十層節點的平均垂直間距由 96px 提高至 132px，逐層高度偏移同步放大，使跨層路線不再擠成近水平線。
- 左右安全邊界向內增加，節點群縮小水平跨度並集中於畫面中央；水平漂移略微收斂，仍保留由 Run seed 決定的不規則感。
- 既有節點外緣端口、分支錯開與生成端不交叉規則維持不變。

## 2026-09-18：主頁／地圖美術、路線與鎖定回饋修正

- 新增原創 `observatory_map_v2.png`，主頁與地圖改用和戰鬥儀式場一致的像素比例、午夜藍／黃銅色盤與浮空觀星城世界；舊港口背景僅保留來源紀錄。
- 程序路線先建立單調、不交叉的連線骨架，再補通過交叉檢查的分支；畫面依 Run seed 加入逐層水平漂移、節點細微偏移與不等距層高，使十層地圖不再像固定格紙。
- 地圖路線改為像素斷線，從節點外緣的獨立端口連接；線條不穿過節點與文字，共用來源／目標的分支也不再疊在同一條線上。
- 手牌選取改為四角鎖定記號；敵人鎖定框移至立繪上方的獨立覆蓋層，每次選取與版面變更都重算座標，首擊即可置中且不被名稱／HP 壓住。
- 消除流程改為先逐一完成全部咒文名稱與效果結算，再讓所有消除格同步閃白並清除。
- 構築頁移除石板綁定說明句，石板預覽、咒文名稱與完整規則改為卡內垂直置中，整體網格水平置中。

## 2026-09-16：角色、護盾與場景融合修正

- 以圖片編修及 chroma-key 去背流程建立實色 `investigator_chibi_opaque.png`，取代先前因去黑底造成角色內部像素半透明的版本。
- 玩家與敵人整組向下校正，使腳底落在背景儀式平台，而非漂浮於後方欄杆與天空層。
- 玩家及敵人 HP 條加入青色護盾疊層；護盾覆蓋 HP 尾端並在同一條狀態文字顯示數值，滿血時仍可看見。
- 四格長石板在側邊手牌縮放時，符文的字級、角標、邊框、圓角及位置會同步縮放；拖曳預覽改為以 46px 棋盤原尺寸重建格子與符文。
- 全畫面背景降低亮度與飽和感並加深藍色分級；常態 Label、按鈕與數值文字加入深色描邊，提高複雜背景上的辨識度。
- 石板桌底色完全透明，手牌、棋盤、AP、回合與按鈕直接落在背景書桌區，不再形成獨立矩形面板。
- 回歸測試新增護盾疊層、手牌符文縮放與拖曳原尺寸斷言。

## 2026-09-15：單一連續戰場背景與角色比例調整

- 主場景改為只使用一張 `ritual_arena_v2.png` 全畫面背景；移除戰場內的第二張裁切背景，以及已透明但仍留在節點樹中的舊中景／前景圖片。
- 同一張圖現在由上方星空儀式平台連續延伸至下方調查桌，角色與棋盤不再位於視覺上斷開的兩張場景。
- 玩家向畫面中央移動並放大至 200×240 顯示區；敵人卡寬度與立繪高度同步提升，五名敵人仍能維持單列與左右順位。
- 石板桌移除所有外框線，只保留低透明度暗底協助辨識棋格，讓操作區與背景桌面自然融合。
- 以 1920×1080 戰鬥擷取確認單張背景連續性、角色尺寸、鎖定四角與完整棋盤／手牌皆無溢出。

## 2026-09-15：側邊手牌與戰鬥資訊層級調整

- 將三個固定手牌槽改為棋盤左側垂直排列，使石板區高度由手牌與棋盤相加改為取兩者最大值，釋放更多高度給 2.5D 戰場。
- 放大敵人立繪與名稱、HP、意圖文字；敵人意圖改為常駐「分類圖示＋短名稱」，並在戰場上方保留完整分類圖例與 tooltip。
- 鎖定標示改為直接依立繪矩形繪製四個像素角，不再依容易被容器拉伸的完整卡片外框定位。
- AP 改為操作區左上大型數字，回合／時限移至右上；大型結束回合按鈕固定於棋盤右下，敵方回合會沿用既有禁用規則。
- 石板桌透明度降低以露出後方場景；全域內文／說明基準字級提高，手牌名稱與摘要同步放大。
- 戰鬥浮字加入 28px 像素字、粗暗色外框、彈出縮放、停留與上飄淡出，HP、護盾、Sanity 與狀態變化更易辨識。

## 2026-09-15：戰場／石板桌分離與 1080p 收斂

- 依實機畫面回饋，移除戰鬥與棋盤共用的大型面板；調查員與敵人改為共處上方 2.5D 儀式圓台，保留由左至右的敵人位置語意與鎖定光環。
- 新增原創 `ritual_arena_v2.png`，以遠景拱門、中景地磚、前景桌面建立景深；石板區收為下方獨立、置中的像素風石板桌。
- 棋盤移除行列刻度；棋格、符文與手牌依 1080p 空間重整，四格長石板現在同時受寬高限制並完整留在固定手牌槽內。
- 常態介面統一使用繁中像素字型，密集效果資訊仍保留為可由元件覆寫的閱讀層；狀態讀數縮至角色旁，不再壓出戰場。
- 以 Godot 內建 UI capture 驗證五敵人戰鬥畫面、獨立石板桌與三張手牌皆在 1920×1080 內可見。

## 2026-09-13：Phase 17 UI、操作與視聽完成度

- 完成現有地圖、戰鬥與結算的實機畫面稽核，新增 `UI_UX_AUDIT_2026-09-13.md` 與固定狀態擷取工具，保留後續視覺回歸入口。
- 建立資料化 UI Theme，統一深色調查風格的背景、面板、按鈕、焦點框、tooltip、字級、間距與 HP／Sanity／MP 狀態色。
- 重組戰鬥畫面並建立正式玩家 HUD 與敵人卡；AP、戰鬥時限、資源、護甲、意圖、狀態、鎖定、支付預覽及回合操作可在 1920×1080 同屏查看。
- 方塊新增名稱與 1–3 快捷鍵；核心盤面支援 WASD 移動、Space 放置、X 取消與非法位置紅色預覽，同時保留滑鼠拖曳，並遵守不可手動旋轉的既有規則。
- 地圖加入進度、圖例、節點圖標與焦點路徑；事件、獎勵、構築檢視、結算、新遊戲、暫停及設定畫面完成一致化改造。
- 獎勵比較直接顯示咒文效果、MP、稀有度、Tier 與持有數；構築畫面顯示咒文／方塊池，主動移除因影響平衡登錄為 `UX-007`。
- 建立 Music／SFX bus、AudioManager 與資料化動效時間介面；資料驗證與 19 組 headless 測試通過。

## 2026-09-13：編輯器新局、隨機地圖、教學與 Run 歷史

- 修正 `run_config.seed = 424242` 被一般 runtime 直接使用的問題；新增 `RunSeedPolicy`，一般新 Run 使用隨機正整數 seed，連續重開不會沿用上一局，固定模式只供除錯與模擬。
- Godot 編輯器執行預設建立新 Run、刷新進行中自動槽；正式匯出版本維持有效存檔續玩，headless 測試以獨立目錄繼續驗證恢復流程。
- MetaState 升至 v3，最近 20 局另存勝敗、原因、Sanity 死因、seed、節點／戰鬥進度、剩餘 HP／Sanity／MP／金錢與結束時間，並保存首次教學進度；v1／v2 profile 可連續自動遷移。
- 地圖提示直接顯示本局 seed，方便玩家回報與重現；內容驗證新增 seed 模式、編輯器啟動策略與歷史上限欄位。
- 戰鬥時限教學改為資料化、跨局只顯示一次的非阻斷訊息；Sanity 報告新增依來源彙總損失與實際致死來源，正式結算、近期 Run、單場及完整 Run 模擬共用分類。
- 自動測試新增隨機／固定 seed 策略、歷史上限、round-trip、摘要嚴格驗證、教學持久化、Sanity 死因與 Meta v1→v3 遷移案例。

## 2026-09-09：時間壓力、10 層地圖與稽核方案落地

- 將程序與 fallback 地圖延長為 10 層，每條路線固定 7 場戰鬥；新增第二個事件、商店咒文商品及兩組後段 encounter。
- 完成資料化戰鬥時限：依 encounter 強度與節點深度計算 4–7 回合；逾時每回合扣除遞增的 2／4／6… Sanity，HUD、統計與模擬器同步支援。
- 節點 MP 回復提高至 50，使 Sanity 壓力主要來自拖延；消除預覽新增咒文觸發、MP／Sanity 支付與致死警示，並加入可收合咒文圖例。
- 依玩家回覆完成 `BOARD-007` 與 `COMBAT-009`：菁英候選保底尚未取得的特殊形狀；六名敵人依自身 HP 或回合切換可預告意圖。
- 內容驗證擴充條件式意圖、時間壓力與節點授予；自動測試涵蓋時限扣除、條件意圖、菁英保底、商店咒文與 10 層地圖。
- 五組種子共 40 Runs 的最佳代理通關率為 77.5%，HP／Sanity 失敗 5／4，平均時間壓力損失 9.5 Sanity；8 組 encounter 共 240 場獨立測試皆可完成。

## 2026-09-08：咒文辨識、MP 代付與多種子體驗稽核

- MP 不足時改用相同數量的 Sanity 支付並照常觸發咒文；戰鬥、完整 Run 與批次統計獨立記錄 Sanity 代付，不再算作 fizzle。
- 20 種咒文加入獨立盤面圖標與效果分類色；手牌及落盤格都能直接辨識，不需先使用 hover tooltip。
- 移除所有常規咒文的直接治療與玩家再生，原回復咒文改為護甲／堅硬；HP 回復保留給商店、休息與高代價節點。
- 敵人重新提高傷害與攻擊意圖比例，並適度延長 HP，使意圖在敵人死亡前有實際作用；自動測試同步更新。
- 新增 `tools/analyze_gameplay.gd`，以五組種子執行 40 次完整 Run 與 180 場獨立戰鬥；最佳策略代理通關率 85%，4 次 HP、2 次 Sanity 失敗，0 次超時。
- 新增 `GAMEPLAY_AUDIT_2026-09-08.md`，整理難度、節奏、豐富性與體驗問題，並把特殊形狀保底及條件式敵人意圖兩個高影響方向登錄為 `BOARD-007`、`COMBAT-009`，未確認前不實作。

## 2026-09-08：版本化設定與共享 Meta 存檔

- 新增獨立 `SettingsState` 與 `MetaState`：設定保存繁中／英文語系、全螢幕、三類音量及匿名回報偏好；Meta 保存共享貨幣、Run 統計、職業／方塊／咒文解鎖。
- 新增 `ProfileSaveService`，讓 `settings.json` 與 `meta_progress.json` 各自使用版本 envelope、原子暫存寫入、last-known-good 備份、損壞回退及 legacy raw payload 包裝。
- 新增 `meta_progression.json` 初始白名單；目前涵蓋既有 1 個職業、11 種方塊與 20 種咒文，確保接入 Meta 後不改變現行 Run 平衡。
- RunManager 啟動時載入／建立 profile，套用語言、視窗與音量；新 Run 起始池及獎勵候選會受到共享解鎖限制，失效 Meta 內容引用則改用記憶體預設且不覆寫來源。
- 新 Run 與結算會保存共享統計；另提供經內容驗證的職業、方塊、咒文解鎖入口，但未自行新增第二職業或永久 HP／Sanity／AP 成長。
- 測試增至 19 suites，新增設定與 Meta round-trip、備份回退、嚴格值域、解鎖去重、內容引用與獎勵過濾案例。

## 2026-09-08：資料化商店與休息節點

- 新增 `shops.json` 與 `rests.json`，商店提供 HP／Sanity／MP 補給及離開選項；Boss 前休息提供完整身心休養、回滿 MP 或直接離開。
- 事件、商店與休息共用選項解析器及既有介面，支援 HP、Sanity、MP、金錢的成本／結果預覽、資源不足禁用與上限裁切。
- RunManager 不再直接結算商店或固定全回復休息；三種非戰鬥節點皆由內容 ID 載入並透過 `RunNodeResult` 結算，之後再套用固定節點 MP 回復。
- ContentRegistry 與 Python 驗證新增商店／休息檔案、模板、地圖內容池及 RunState 地圖引用檢查。
- 主場景測試新增實際購買魔力瓶與安穩休養流程；完整 Run 模擬器亦改為選取第一個可負擔的非戰鬥選項。
- 固定 seed 10-run 基準維持 80% 通關、0 次 HP／Sanity 失敗、2 次超時；結束平均 HP／Sanity／MP 為 `74.5/95.0/47.3`。

## 2026-09-07：效果格核心、統一咒文與 MP 正式遷移

- 依 `CORE-002` 至 `CORE-005` 完成核心改版：保留 Row／Col 消除，方塊抽取時獨立附著一個咒文與一個 ✦ 效果格；交叉消除同格只觸發一次。
- 移除正式 Run 的 Row／Col 裝備、背包與配置流程；`items.json` 改為 `spells.json`，20 個效果全部重新命名及設定 MP、範圍、傷害、護甲、治療或狀態。
- 實作 MP 70/100、每完成節點回復 35、MP 不足仍消除但咒文失效；戰鬥統計與批次工具加入 MP 消耗、觸發與 fizzle。
- RunState 升至 v5，保存 MP、盤面咒文、手牌咒文／效果格及咒文池；v4 Row／Col 裝備可遷移為咒文池。
- 特殊形狀改為每種只解鎖一次；星點石板權重降至 0.16、智慧加成降至 2，同手在種類足夠時維持形狀 ID 唯一。
- 敵人基礎攻擊下修至 5–8，重擊倍率 1.35，意圖增至 8 種並加入窺視與凝神防備，降低連續傷害。
- Reward tier 門檻同步校正為 65／120，避免降傷後讓特殊形狀的既有解鎖階段意外延後。
- 固定 seed 10-run 基準達 80% 通關，0 次 HP／Sanity 失敗、2 次超時；完整資料、18 組測試與一鍵模擬均通過。

## 2026-09-07：實玩回饋修正與核心玩法評估

- 修正特殊方塊在智慧手牌中機率被旋轉候選與重複池項目過度放大的問題；候選改為同 ID 合併後依 `weight × 池內份數` 抽取，池內類型足夠時同手不再出現相同方塊 ID。
- 同步修改 runtime 與無 UI `BoardSimulator`，新增單份／雙份星點石板分布、三張唯一 ID 及小型池允許保底重複的固定 seed 回歸測試。
- 更新 `COMBAT-004`：鎖定敵人死亡後自動切換 encounter 原始順序第一名存活敵人；同次放置後續效果沿用新目標，批次模擬器亦採相同規則。
- 六名敵人基礎攻擊由 `8/10/12/10/8/14` 下修為 `7/8/10/8/7/11`，並讓四名原本意圖較單調的敵人穿插自身強化、普通攻擊或重擊，降低連續固定傷害。
- 將 reward Tier 2 強度門檻由 80 微調為 75，抵銷降傷造成的第二場獎勵降級，維持原定成長開放節奏。
- 最新單場批次前五場皆為 100% 勝率，第六場 90%；完整 Run 由 0% 提升為 10%，且沒有 HP 死亡，但 9/10 仍因 Sanity 失敗，後續資源問題集中到已決定的 MP 方向。
- 玩家提出將觸發效果與裝備改綁方塊效果格。評估此方向可降低「放方塊還要記行列裝備」的認知負擔，但會影響棋盤狀態、內容 schema、獎勵、存檔與 UI；新增 `CORE-002` 至 `CORE-005`，確認前不直接重寫核心。

## 2026-09-07：Phase 18 存檔基礎與可重現狀態

- 再次檢查 `OPEN_QUESTIONS.md`；`MAGIC-001`、`CONTENT-003`、`CONTENT-007` 的玩家回覆仍為「待回覆」，因此未改動 MP、商店／休息或正式視聽方向。
- 新增獨立 `SaveGameService`，提供單一 Run 自動槽、版本 envelope、可注入路徑、同目錄暫存、原子提升、last-known-good 備份與損壞主檔回退。
- 存檔載入先做嚴格結構與數值範圍檢查；不支援的版本、錯型或無法遷移的現有主檔會被拒絕且不遭覆寫。
- RunState schema 升至 v4，保存獎勵與抽牌 RNG 當前 state；舊 v1–v3 快照可逐版遷移，v3 會以 Run seed 建立穩定 RNG 初始狀態。
- 新 Run 的 Tablet RNG 改由 Run seed 加固定 salt 初始化；記憶體快照擷取／還原會同步保存兩套 RNG，避免讀檔後改變後續獎勵或手牌序列。
- 自動測試增至 18 suites，新增磁碟 round-trip、暫存檔清理、備份輪替、壞主檔回退、拒絕覆寫、v3 遷移、嚴格資源範圍及 RNG 延續案例。
- 將服務接入 RunManager：新 Run、節點入口、節點完成與結算均自動保存；啟動時會繼續有效存檔，`node_entered` checkpoint 恢復時固定重啟已選節點，不能回地圖重選。
- 若主檔的結構或內容引用失效，RunManager 會再驗證備份；成功回退後隔離原始壞檔為 `.rejected-*`、修復主槽並繼續保存。envelope 同時記錄各內容文件 schema 版本，預留 content-ID 遷移。
- 依審查補強合法色碼、背包數量、歷史項目、手牌雙欄一致性、AP 與終止資源狀態、checkpoint、時間戳及 JSON 安全 seed 範圍驗證；暫存提升前也會完成整份 round-trip 驗證。
- `tools/validate_all.sh` 已通過 Python 資料驗證、18 組 Godot 測試、棋盤、單場戰鬥、完整 Run、Sanity 與 encounter 報告；完整 Run 仍為 0% 通關的舊基線，因 `MAGIC-001` 未決，本次未自行調整核心數值。

## 2026-09-06：最新決策整理與資料化事件

- 將玩家對 Meta、存檔、MP、可及性、內容及發布問題的明確回覆移入已決定；`META-002` 依原選項語意正規化為以角色差異取代永久 HP／Sanity／AP 成長。
- 將尚未覆蓋的重大細節拆為 `MAGIC-001`、`CONTENT-007`，連同視聽資產 `CONTENT-003` 保留待確認，不先改動核心資源或商店／休息方向。
- 新增 `events.json` 與事件範本，第一筆原創事件提供三個選項；HP、Sanity、金錢代價與結果皆由資料定義，兩套驗證器會檢查值域、選項 ID 與地圖引用。
- 新增純資料 `EventChoiceResolver`、事件畫面與 RunManager 接線；玩家可先看成本／收益，資源不足選項會禁用，事件完成後回到地圖且保留盤面。
- 事件 Sanity 變化保留來源記錄，HP／Sanity 不可支付至 0，回復受上限限制；上述小型邊界規則已寫入內容文件。
- 自動測試增至 17 suites，涵蓋事件預覽、結算、上限、不足、未知選項、UI 與 Run 流程；完整一鍵驗證結果記錄於本次交付說明。
- `tools/validate_all.sh` 已通過 Python 資料驗證、17 組 Godot 測試、棋盤、單場戰鬥、完整 Run、Sanity 與 encounter 報告；未調整既有戰鬥數值，完整 Run 基線仍為 0% 通關。

## 2026-09-06：專案盤點與 Phase 16 內容完整性

- 重新檢查使用者更新過的 `OPEN_QUESTIONS.md`；目前待確認列仍全部標示「待回覆」，因此未將候選方向誤當成新決策。
- 完整對照場景、GDScript、JSON、測試、Roadmap 與企劃文件；確認可玩進度已到 Phase 15 完成、Phase 16 進行中，並校正 Roadmap、核心迴圈與開發指引中的過時現況。
- 整理 `OPEN_QUESTIONS.md`：將玩家明確回覆的 `UX-003`、`CONTENT-004`、`RELEASE-003` 移入已決定，其餘候選方向仍保留為待確認；新增 `CONTENT-005` 追蹤公版／授權邊界。
- Python 與 Godot 內容驗證新增升級鏈雙向對應、tier／rarity／軸連續性、循環防護、獎勵前置與各 tier 候選容量、地圖內容池、DAG 與唯一 Boss 終點規則。
- `ContentRegistry` 新增 RunState 內容引用驗證；`RunManager.restore_run_state()` 在修改 runtime 前拒絕遺失的方塊、道具、獎勵、地圖或 Sanity ID。
- 修正 `COMBAT-004` 實作落差：鎖定敵人死亡後現在會保持空目標，單體／擴散效果在玩家手動選擇下一名敵人前失效，並加入多敵人整合測試。
- 新增固定 seed 的 `BoardSimulator` 與 headless 報告工具，依現行補牌、AP、消線與死盤重建順序，比較智慧手牌和權重隨機的合法率、消線率與盤面佔用；同 seed 結果由測試保證可重現。
- 新增無 UI `BattleBatchSimulator`，以相同棋盤事件驅動現有 Entity、道具 Resource、敵人意圖、狀態時序、目標與勝敗規則，輸出六場遭遇的勝率、回合、傷害、護甲、Sanity 與敵人存活曲線。
- 新增六種現行註冊內容的 schema v1 JSON 範本與 `CONTENT_SCHEMA.md`，建立穩定 ID、欄位廢棄、breaking change、RunState 引用和遷移順序規範；正式事件／商店／休息 schema 因影響互動方向，新增 `CONTENT-006` 待玩家確認。
- 新增 `tools/validate_all.sh`，把 Python 資料驗證、16 組 Godot 測試、棋盤、單場／完整 Run 戰鬥、Sanity 與 encounter 基線整合為單一 headless 入口，並額外攔截 Godot 偶爾未以非零狀態回報的 script parse error。
- 新增持久化棋盤 session 與 `RunPressureSimulator`，讓固定 seed 路線跨戰鬥保留手牌、盤面、HP、Sanity、方塊池、獎勵、背包合成與裝備；測試增至 16 suites，一鍵驗證納入完整 Run 批次。
- 10 組完整 Run 基線通關率為 0%；9 組因 Sanity、1 組因 HP 失敗，第三場抵達後勝率僅 40%。此結果已登錄為 `BALANCE-001`，未在玩家選擇主要調整方向前自行修改核心數值。
- 自動測試增加到 16 suites；`python3 tools/validate_data.py`、棋盤／單場戰鬥／完整 Run 基線模擬與 Godot 4.5.1 headless 整合測試全部通過。本次未修改遊戲數值。

## 2026-07-18：Phase 15 與地圖可讀性

- 將程序地圖改為可捲動圖形畫布，依 `next_ids` 繪製全部連線，並以灰／黃／綠標示一般、可選與已走路線。
- 將事件、菁英、商店、休息指定為完整樓層，生成路線只保留四層普通戰鬥；完整路徑仍維持 6 場戰鬥。
- 新增資料化 Sanity 階段、跨系統瘋狂效果池、統一來源介面、HUD 歷史與魔法後果預覽。
- RunState schema 升至 3，保存瘋狂效果與 Sanity 來源歷史。
- 新增完整 Run Sanity 壓力模擬與第 12 組自動測試。

## 2026-07-18：Phase 14 完整 Run 與程序地圖整合

- 整理 `BOARD-003`：所有 Run 節點間完整保留盤面，只有全新 Run 重建盤面。
- 整理 `BOARD-005`、`BOARD-006`：方塊抽入手牌時由系統決定旋轉方向；玩家不能手動旋轉；回合末保留未使用手牌與方向，下一回合只補足缺額。
- 參考使用者提供的 Godot 4.5 `114-2_dsap_project`，移植分層 DAG、距離連邊、交叉抑制與可達性驗證；改為注入 Run seed、現有節點類型與 schema。
- `data/map.json` 新增程序生成參數、節點權重與 encounter 內容池；同 seed 可完全重現。
- 路線保證多起點、事件、商店、菁英、Boss 前休息、單一 Boss，以及每條路徑 5–8 場戰鬥。
- 新增可操作地圖 overlay 與結算／重新開始畫面；RunManager 正式接入七個流程狀態。
- 所有節點透過 `RunNodeResult` 修改 RunState；盤面、手牌方向、裝備、玩家資源、背包、金錢、生成地圖與完成節點均可序列化。
- 普通／菁英勝利進入獎勵後返回地圖，Boss 勝利直接通關，失敗進入結算；移除六場 encounter 的 modulo 無限循環。Phase 14 全項完成。

## 2026-07-18：Phase 13 收束與 Phase 14 基礎

- 整理玩家文件回覆：`BOARD-001` 每手至少一張有方向、`BOARD-002` 程序友善盤面、`BOARD-004` 重複特殊方塊增加抽取權重，並移入已決定區。
- 新增 `FriendlyBoardGenerator`，以 JSON 的格數及 Row／Col 缺口參數建立不會開局直接消除的友善盤面。
- 智慧手牌評分新增 `direction_score`，只把方塊實際參與的接近完成線視為方向；每手第一張優先直接消除或有方向候選。
- 特殊方塊重複取得會保留重複池項目，提高實際抽取權重；獎勵 tooltip 與結果顯示持有份數。
- Phase 10B 與 Phase 13 全項完成。
- 開始 Phase 14：新增 `RunStateMachine`、`RunNodeResult`、版本化 `data/map.json`，建立雙起點、中段分支、Boss 前休息與單一 Boss 的 7 節點固定路線。
- ContentRegistry 與 Python 工具驗證節點型別、引用、邊、起點、Boss，以及所有起點到 Boss 的可達性；RunState 加入流程與節點欄位。
- 自動測試擴充為 11 suites；Phase 14 後續接線等待 `BOARD-003`。

## 2026-07-18：戰鬥操作修正與 Phase 13 局內成長

- 死亡敵人不再保留灰色卡片；roster 只建立存活敵人卡並保存原始敵人索引，死亡時清除舊鎖定並選到第一名存活敵人。
- 方塊拖曳成立後隱藏手牌原位置的圖形，只保留拖曳預覽；取消拖曳會恢復來源，成功放置仍移除來源節點。
- `BattleItem` runtime 加入 rarity、tier、balance cost、升級來源／目標及合成數量；新增三條 JSON 管理的 Tier 1 → Tier 2 合成鏈。
- 新增 `EquipmentGrowthService`；道具先進背包，相同道具依 JSON 合成，戰鬥外可比較八個合法槽位後選擇裝備或保留。
- 新增 encounter 強度模型與 reward tier；候選器支援權重、稀有度／tier、最小勝場、解鎖、去重及固定種子重現，並加入跳過換局內金錢。
- 新增逐場戰鬥統計與固定六場平衡報告工具；測試擴充為 10 suites。
- Phase 13 除特殊方塊／手牌成長外均完成；該項等待 `BOARD-001`、`BOARD-004`、`BOARD-006` 決策。

## 2026-07-18：Phase 12 戰鬥系統收束

- 確認並整理 `CORE-001`、`COMBAT-001` 至 `COMBAT-008`；正式 slot 分類改為 Row 物理／Col 魔法，同時死亡判玩家失敗，敵人完整支援六狀態，encounter 上限為 5 名。
- 新增版本化 `data/intents.json` 與 `EnemyIntentState`／`EnemyIntentExecutor`。每名敵人獨立循環意圖，並依隱藏速度由高至低行動；未知 ID 或 action 會驗證失敗。
- 實作攻擊、重擊、防禦、Sanity 攻擊、玩家 debuff 與敵人 buff 六種資料化意圖；新增敵人 `speed` 並讓無光祭司、深淵守衛覆蓋新意圖。
- 固定回合時序：玩家回合末狀態、敵人依序行動與各自回合末狀態、完整輪統一衰減、下一玩家回合。
- 新增 `TargetResolver`，落實擴散跳過死亡空位與鎖定目標死亡後不自動重選。
- 新增 `BattleEffectContext`，提供觸發軸、索引、目標集合、觸發歷史及戰鬥狀態；連發手槍透過 context 判斷條件，`BattleManager` 不再保存個別道具特例。
- 新增 `BattleOutcomeResolver`，統一勝利、HP／Sanity 失敗與同時死亡優先序。
- 新增可重用 `EnemyCard`；五敵人 roster 只在成員改變時調整節點，HP、護甲、狀態、意圖與鎖定更新不重建卡片。
- 測試擴充為 8 suites，涵蓋資料、RunState、棋盤、意圖與速度、目標解析、狀態與結果、五敵人 UI，以及完整敵人回合 smoke test。
- 新增 `docs/COMBAT_SYSTEM.md`，並同步更新 Roadmap、資料指南、數值紀錄與 TODO。Phase 12 全項完成。

## 2026-07-18：Phase 11 架構與資料基礎

- 建立 `tests/` 正式測試入口，涵蓋 ContentRegistry、RunState round-trip、BoardModel 與 `main.tscn` 核心迴圈 smoke test。
- 新增 `ContentRegistry`，集中處理七份 JSON 的載入、schema 版本、值域、唯一 ID、跨檔案引用、資料查詢與 runtime Resource 建立；`RunManager` 不再直接解析 JSON。
- 所有資料檔加入 `schema_version: 1`；`tools/validate_data.py` 同步驗證資料根節點、intent、rarity、scope、status、tier、slot 與引用。
- 建立最小可序列化 `RunState`，保存玩家資源、AP、盤面、手牌、方塊池、裝備、進度、獎勵與種子，並提供現有 runtime 的擷取／還原介面。
- 新增 `BattleStartInput`、`BattleResult` 與 `battle_finished`，讓 Run 流程透過資料介面開始／結束戰鬥，不由戰鬥控制器決定下一個場景。
- 將敵人建立與原型 UI 分別抽到 `EnemyFactory`、`EnemyRosterPresenter`；將盤面規則與智慧手牌評分抽到 `BoardModel`、`SmartHandScorer`。
- 新增獨立戰鬥控制器、獎勵、地圖容器與結算場景；現有主場景仍維持可玩流程，地圖／正式結算留待 Phase 14 接線。
- 新增 `docs/ARCHITECTURE.md` 記錄責任與資料流。
- 玩家確認 `ARCH-001`：JSON 管內容組合與數值、`.tres` 管效果行為。新增四個正式效果原型與 `run_config.effect_resources` 對應，`ContentRegistry` 改為複製 `.tres` 後套用 JSON 數值。
- 將連發手槍的條件傷害從 `BattleManager` 特例移入 `EffectConditionalAttack`，讓效果行為由 `.tres` 選定的策略類別負責。Phase 11 全項完成。
- 整理 `OPEN_QUESTIONS.md` 的歷史錯置欄位：將已填答的 `CORE-001`、`COMBAT-001` 至 `COMBAT-005`、成長、獎勵、Run、節點、Sanity、`UX-001` 與 `UX-002` 正規化後移入「已決定」，並保留仍只有候選方向的問題。

## 2026-07-17：專案完成性盤點與後續規劃

- 以目前程式、場景、JSON、企劃文件與資料驗證結果為基礎，完成從戰鬥原型到可發布版本的整體完成性盤點。
- 新增 `docs/PROJECT_ROADMAP.md`，將後續工作分為架構與資料、戰鬥收束、局內成長、完整 Run、Sanity、內容工具、UI、存檔／局外及發布準備九個獨立章節。
- 每個 Roadmap 章節都記錄現況、目標、前置條件、執行內容、交付成果、驗收標準與相關 Open Question ID。
- 重整 `docs/OPEN_QUESTIONS.md`；已回答規則移入「已決定」，真正未決內容依架構、核心分類、棋盤、戰鬥、成長、獎勵、Run、節點、Sanity、Meta、存檔、UX、內容及發布分類。
- 將 `Codex/TODO.md` 改為精簡執行索引，保留 Phase 1–10 歷史摘要，新增 Phase 11–19 並連結 Roadmap 與設計閘門。
- 明確保留壓力資源原則：普通戰鬥獎勵不提供 HP／Sanity 回復或上限、AP 上限；相關內容即使存在，也必須是非常珍貴的稀有獎勵。
- 確認目前主場景仍可由 Godot 4.5.1 headless 載入，資料驗證通過；後續可先從正式測試、資料註冊層、RunState 與既有欄位驗證開始，不需等待企劃問題回答。

## 2026-07-04

- 閱讀目前專案文件與程式結構。
- 開始 Phase 4，新增可重用的 `Entity` 腳本，包含 HP、最大 HP、護甲、受傷、治療、加護甲與死亡信號。
- 將 `BattleManager` 的 dummy 戰鬥對象替換為場景中的 Player 與 Enemy 實體參照。
- 將 Player 與 Enemy 數值連接到右側 UI 文字。
- 更新攻擊與支援道具，讓 Row / Col 消除後能真正改變戰鬥數值。
- 修正方塊場景載入路徑大小寫，從 `Block.tscn` 改為 `block.tscn`。
- 根據設計回覆整理 `OPEN_QUESTIONS.md`，之後未決問題與開發紀錄改用繁體中文。
- 開始 Phase 5，將測試用方塊生成改為正式手牌抽取流程。
- 成功放置方塊後會消耗該手牌方塊；手牌耗盡時自動抽出新的一手。
- 同時消除 Row 與 Col 時，觸發順序改為先 Col 再 Row。
- 開始 Phase 6，加入 `PLAYER_TURN` / `ENEMY_TURN` 回合狀態。
- 玩家 AP 由資料表控制；每成功放置 1 個方塊扣 1 AP，AP 歸零後暫停棋盤放置。
- `EndTurnButton` 現在會清空玩家護甲、切換到敵人回合、執行敵人攻擊，然後回到玩家回合並刷新手牌。
- 敵人意圖目前以固定攻擊作為原型，顯示在 `IntentLabel`。
- 依照戰鬥原型需求，新增目前裝備摘要文字，只顯示 Row / Col 對應裝備，不提供戰鬥中修改功能。
- 修正護甲重置時機：改為玩家回合開始時清空，確保同一回合獲得的護甲能防住敵人攻擊。
- 修正 AP 歸零後的互動：現在會直接禁用手牌拖曳，而不是拖曳後才拒絕放置。
- 新增 Sanity 數值：玩家上限 100、初始 70；Curse 物品預設消耗 3 點 Sanity，`暗影刺` 已設定成本。
- 進入 Phase 7，加入狀態 Label 閃色、浮動戰鬥文字與方塊拖曳縮放 / 禁用外觀。
- 進入 Phase 8，新增 `RunManager` 作為單局流程控制器。
- 新增 `data/enemies.json`、`data/rewards.json`、`data/run_config.json`，先用 JSON 管理敵人、獎勵池與起始配置。
- 敵人死亡後會進入勝利狀態，禁用棋盤與結束回合按鈕，並顯示三選一獎勵。
- 玩家 HP 或 Sanity 歸零會進入失敗狀態；Sanity 低於 30 時會在回合 UI 顯示「理智不穩」。
- 選擇方塊獎勵會加入後續抽牌池；選擇裝備獎勵會替換指定 Row / Col 槽，然後直接開始下一場戰鬥。
- 棋盤邊緣現在會顯示 Row / Col 類型與目前裝備，滑鼠停留時顯示道具效果。
- 拖曳方塊時，若該位置會造成行列消除，預計被消除的整條 Row / Col 會以高亮預覽。
- 將玩家、方塊、道具、敵人、獎勵與初始配置整理為 `data/` 內的 JSON；`RunManager` 會從 JSON 建立 runtime Resource。
- 新增 `data/README.md` 說明新增方塊、道具與獎勵的資料格式。
- 新增無合法步懲罰：若目前手牌沒有任何方塊能放到盤面上，會扣 10 Sanity，清空盤面並重抽手牌。

## 2026-07-17

- 撤回常規 HP / Sanity / AP 成長獎勵；HP / Sanity 回復與上限、AP 上限改列為非常稀有的特殊節點獎勵，不進入普通戰鬥三選一。
- Phase 10 保留特殊方塊獎勵入口；玩家成長曲線改回待設計，方向以裝備升級、稀有度、特殊方塊與手牌規則為主。
- 新增特殊方塊：星點石板、雙星石板、十字石板；特殊方塊可透過 `weight` 與 `smart_score_bonus` 影響抽牌與智慧手牌。
- 完成智慧手牌 v1：抽牌時會評估目前盤面，優先給能直接消除或補接近完成行列的方塊，同時保留前幾名候選的隨機性。

## 2026-07-06

- 改善盤面節奏：開局與死盤懲罰後會生成友善盤面，不再從完全空盤開始。
- 進入下一場戰鬥時保留目前盤面，只重置敵人、手牌、AP、護甲與暫時狀態。
- 玩家身上的異常狀態會在新戰鬥開始時清除，但 HP / Sanity 仍保留。
- 將智慧手牌、特殊方塊構築與玩家成長曲線列入 Phase 10 待辦與設計文件。
- 補齊 Phase 9 內容基準：目前方塊 8 種、道具 17 件、敵人 6 種、遭遇 6 組。
- 新增 `tools/validate_data.py`，可檢查 JSON ID、獎勵引用、Row / Col slot 合法性、道具 / 敵人最低數量與 encounter 敵人引用。
- 改善裝備效果查看：棋盤邊緣、右側目前裝備清單與獎勵按鈕都會顯示道具效果 tooltip。
- 新增 Equipment：加固長外套、護身臂甲。
- 新增敵人：墓園教徒、扭曲侍僧、深淵守衛。

## 2026-07-05

- 調整資料規則：普通方塊不再作為獎勵，敵人移除 `reward_tier`，獎勵級別改由未來關卡資料計算。
- 新增多敵人初步實作：`encounters.json` 可定義一場戰鬥的敵人組合，戰鬥中可用右側按鈕鎖定敵人，道具可依單體 / 擴散 / 全體範圍結算。
- 新增異常狀態規格與初步實作：力量、虛弱、堅硬、脆弱、再生、中毒。
- Prayer 現在與 Curse 一樣會消耗 Sanity；`聖光術` 改為 cost 3。
- 新增 11 件 common 道具資料：霰彈槍、連發手槍、微型炸藥、狙擊槍、單手盾、轉守為攻、祈願、身強體壯、中毒術、劣化、毒滿地。
- 方塊種類由 2 種擴充到 8 種，新增長條、方形、S、Z、J、短條等普通方塊，並加入開局方塊池。
- 新增 `DESIGN_CORE_LOOP.md`、`NUMERIC_MODEL.md`、`CONTENT_GUIDE.md`、`BALANCE_LOG.md`，建立短局快測、Sanity 壓力與強連動道具的企劃 / 數值基準。
- 擴充 `items.json`、`blocks.json`、`enemies.json` 的企劃欄位，加入 tags、rarity、trigger_hint、balance_cost、tier、weight、intent_pattern 等資料。
- 更新 `data/README.md`，補充新增敵人與道具平衡欄位的說明。
## 2026-09-16 — Phase 17.9 石板操作、無框地圖與 Q 版敵人

- 手牌寬度增至 360 px，常駐顯示咒文類型、MP、範圍與受當前狀態修正後的完整機制數值；純敘事描述仍不混入戰鬥速讀資訊。
- 石板拖曳起點新增 22 px 就近吸附，棋盤格縫縮至 2 px，格縫與棋盤邊緣新增最近格放置容許；鍵盤放置規則不變。
- 手牌、拖曳預覽、盤面、獎勵與構築預覽共用磨損石板材質；方塊色盤改為低飽和赭石、青灰、酒紅與舊金色。
- 地圖取消整張不透明視窗，以港城背景直接承載路線、儀式環與低透明節點；主選單改為半透明儀式面板與同一暗金像素 Theme。
- 六名敵人全部換成與調查員一致的大頭短身 Q 版像素稿，原圖與綠幕來源均保留；資料引用改指向 `*_chibi_v2.png`。
- 完成 battle、map、title、reward 的 1920×1080 GUI 擷取；完整資料驗證、19 組整合測試、棋盤／戰鬥／完整 Run 模擬皆通過，完整 Run 固定種子通關率為 80%。
- 後續細修：完整效果已常駐的手牌、戰利品、構築卡與消除預覽移除重複 tooltip；盤面效果格仍保留游標說明。
- 石板明度、飽和度與像素邊緣對比提高；地圖節點移除額外圓環，只保留節點框與路線。
- 死盤流程改為鎖定輸入、短暫停頓並播放全盤斜向紅色脈衝，動畫完成後才扣除 Sanity 與重建盤面。

## 2026-09-16 — Phase 17.8 戰利品定位與戰鬥資訊降噪

- `SlatePreview` 的石板與效果格符文改用同一個實際控制項尺寸計算置中原點，並在預覽框改變尺寸時同步更新，修正戰利品符文偏移。
- 手牌槽擴至 300×146，名稱／類型／MP／效果數值字級同步放大；純敘事描述暫時退出手牌、tooltip、圖例、戰利品與構築頁。
- 消除預覽在沒有觸發咒文時完全隱藏；有內容時使用固定 320 px 裁切區與緊湊單行摘要，大量觸發不再改變棋盤或整體版面尺寸。
- 玩家與敵人護盾由藍色覆蓋改為 HP 條上的白色輪廓區段，保留護盾數字並與藍色 MP 資源明確區分。

## 2026-09-16 — Phase 17.7 手牌、消除預覽與流程介面修正

- 手牌石板改用統一格子縮放並整體放大，固定槽擴為完整可點選區；拖曳預覽仍還原棋盤原始格尺寸與符文位置。
- 棋盤右側新增即時消除預覽，列出當前放置會觸發的全部咒文、狀態修正後數值與 MP／Sanity 合計；咒文改為逐一彈出名稱後才分別結算。
- 結束回合按鈕取消鍵盤焦點，解決 Space 同時放置與結束回合；最後敵人死亡後延長結算等待，角色與敵人再向地面下移。
- 戰利品卡直接顯示完整咒文資訊且隱藏複雜度／格數；戰利品與地圖改用一致的暗色像素框景，地圖金線只標示由已完成節點出發的可走路線。
- 移除「最近理智變化」HUD 與每張咒文重複的 MP 不足代付說明，來源歷史與共通規則仍保留。

## 2026-09-14 — Phase 17.1 介面辨識、永久石板與存檔 v6

- 新增七類資料化咒文符文；咒文個別 `icon_text` 保留相容用途，主要辨識改由分類圖示、分類色與範圍角標負責。
- 手牌改為三個固定佔位槽，名稱與 MP／效果摘要常駐；盤面效果格使用接近整格的大型深色徽章。
- 地圖畫布加入置中容器並依視窗寬度限制；構築頁改為直接繪製石板形狀與效果格。
- `UX-007` 已確認：不提供一般移除，只由商店每節點一次的付費服務與特定事件提供。
- 構築單位改為永久綁定的完整石板；起始池、獎勵、商店、事件、手牌與模擬器共用 `slate_uid + shape_id + spell_id + effect_cell`，抽取時只旋轉整塊石板。
- 獎勵依咒文強度與形狀複雜度配對，同輪不重複完整組合；候選效果格由 reward RNG 決定並保存，讀檔不會重抽。
- RunState 升至 v6。v5 進行中自動槽先封存為 `run_autosave.pre_slate_v5.json` 再建立新局，設定、Meta 與歷史保持原狀。
- 加入調查員、六種敵人、儀式室遠／中／前景與港城地圖／標題背景；所有資產皆為本專案提示詞生成的原創像素圖，來源與完整提示詞記於 `ART_ASSET_PROVENANCE.md`。
- 戰鬥畫面改為玩家／固定棋盤／2×3 敵人陣列。敵人依速度逐一蓄力、命中結算、顯示結果並返回待機，行動期間鎖定輸入，左側保留最近四筆紀錄。
- 完成 title、map、battle、reward、build、settlement 六張 1920×1080 GUI 擷取；五敵人同屏、地圖置中、完整石板預覽與轉場後文字均無截斷。

# 2026-09-20 — 戰鬥 HUD 可見性與資訊精簡

- 修正戰場高度縮減後，玩家 HP／SAN／MP 狀態列落在裁切範圍外而完全消失的問題；狀態列改置於調查員左下側的可視安全區。
- AP 與回合資訊放大並向棋盤兩側拉開，提升戰鬥中快速讀取能力。
- 移除戰場右上角的常駐敵人意圖圖例；敵人自身仍保留意圖圖示與提示。
- 頂部情境標題精簡為「戰鬥」。

# 2026-09-20 — 戰場狀態圖示與版面微調

- 地圖畫布新增頂部安全邊距，最上層節點即使受到種子高度擾動也不會被裁切。
- 玩家資源條移至調查員左側，完全避開角色立繪；棋盤下移，結束回合控制區向右展開。
- 新增玩家與敵人共用的特殊狀態列，以分類色圖示與層數呈現力量、虛弱、堅硬、脆弱、中毒與再生。
- 敵人鎖定四角改為包覆立繪外側的較大範圍，同時縮短單一角線，避免搶占角色畫面。
- 完成 1920×1080、1280×720 戰鬥畫面與地圖畫面擷取驗收。

# 2026-09-20 — 戰場重心與手牌邊界修正

- 玩家特殊狀態列移至 HP 上方，HP、SAN、MP 三條資源列維持連續排列。
- 棋盤改為在三張手牌的總高度內垂直置中，使盤面中心與第二張手牌等高。
- 最末手牌槽遇到四格高石板時，利用既有槽間空隙向上避讓；維持 46px 原始格子尺寸且不再超出畫面底部。
- 玩家立繪、名稱、資源列與敵方陣列整體右移，並縮短敵人排列間距，改善戰場左右重心。
- 加入四格高手牌底緣、狀態列順序、棋盤對齊與戰場位移的自動化驗收。

# 2026-09-20 — 細狀態條與敵人資訊避讓

- 玩家 HP、SAN、MP 與敵人 HP 統一縮為 14px 細條，條內數字同步調整，並在玩家三條資源之間加入 5px 間距。
- 敵人名稱與 HP 列分別移至鎖定角框的上、下方安全區，不再與角線重疊。
- 敵方資訊列略微上移，確保新增間距後的特殊狀態與意圖仍完整留在戰場範圍內。
- 新增資源條高度、條間距及鎖定框避讓的自動化檢查。

# 2026-09-22 — 戰場整體放大與敵我行動時間線

- 玩家、敵人、名稱、狀態列、鎖定角與戰場浮層改由同一個舞台容器統一放大 5%，並整體下移 6px，維持各元素間的原始比例與對位。
- 戰場與下半部的透明交界允許資訊自然延伸，保留敵人名稱、鎖定角與 HP 間的安全留白，同時讓放大後的特殊狀態與意圖完整顯示。
- 行動紀錄改為保留整場戰鬥的可捲動時間線，納入玩家放置石板、咒文觸發、敵我傷害／狀態回饋、死亡與敵人行動結算。
- 我方紀錄統一使用藍色並標示「我方」，敵方紀錄使用紅色並標示「敵方」；紀錄移至右上空景，避免遮住最右側敵人。
- 下半部標題列釋出多餘高度給戰場，棋盤本體的畫面位置維持不變。

# 2026-09-22 — 戰鬥文字可讀性與手牌選取穩定

- 玩家 HP／SAN／MP 與敵人 HP／護盾數字放大至 15px，狀態條高度調整為 18px，並加入深色像素外框；維持細條視覺但不再需要費力辨識。
- 玩家與敵人的特殊狀態圖示字級提升至 20px；角色名稱、行動紀錄與手牌咒文資訊同步調整至適合戰鬥速讀的尺寸。
- 取消手牌在鍵盤選取及開始拖曳時的 1.06 倍縮放，選取狀態只保留四角框與棋盤落點預覽，避免卡片與相鄰版面跳動。
