...
var _growth_service = EquipmentGrowthServiceScript.new()
...
func _simulate_once(content: ContentRegistry, seed: int, max_turns: int) -> Dictionary:
	...
	var row_ids := RunState._strings(run_config.get("row_items", []))
	var col_ids := RunState._strings(run_config.get("col_items", []))
	var inventory := {}
	...
	for node in route:
		...
		if node_type == "rest":
			player_state.hp = int(player_state.get("max_hp", 80))
			player_state.sanity = int(player_state.get("max_sanity", 100))
			rests += 1
			continue
		...
		var battle_result := _battle_simulator.simulate_encounter_with_session(
			...
			intent_index,
			player_state,
			content.get_items(row_ids),
			content.get_items(col_ids),
			board_session,
		...
		if str(reward.get("type", "")) == "block":
			...
			var growth_result := _growth_service.add_to_inventory(reward_id, inventory, content.indexes.get("items", {}))
			upgrades += growth_result.get("upgrades", []).size()
			_auto_equip(str(growth_result.get("item_id", reward_id)), row_ids, col_ids, content)
	return {
		"seed": seed,
		"outcome": outcome,
...
func _auto_equip(item_id: String, row_ids: Array[String], col_ids: Array[String], content: ContentRegistry) -> void:
	var item := content.get_item(item_id)
	if item == null:
		return
	var slots := row_ids if item.axis_type == BattleItem.AxisType.PHYSICAL else col_ids
	var weakest_index := -1
	var weakest_cost := 999999
	for index in range(slots.size()):
		var equipped_definition := content.get_definition("items", slots[index])
		var equipped_cost := int(equipped_definition.get("balance_cost", 0))
		if equipped_cost < weakest_cost:
			weakest_cost = equipped_cost
			weakest_index = index
	var new_cost := int(content.get_definition("items", item_id).get("balance_cost", 0))
	if weakest_index >= 0 and new_cost >= weakest_cost:
		slots[weakest_index] = item_id
...
func _summarize(results: Array[Dictionary], seed: int, max_turns: int) -> Dictionary:
	...
	for result in results:
		...
		for key in ["battles_won", "battles_reached", "rests", "rewards_taken", "upgrades", "special_blocks", "hp_end", "sanity_end", "board_placements", "dead_boards"]:
	...
	return {
		...
		"policy": "隨機合法路線；三選一取第一項；新道具只在不弱於最低 balance_cost 槽時自動裝備",
		...
		"average_upgrades": float(totals.upgrades) / float(count),
